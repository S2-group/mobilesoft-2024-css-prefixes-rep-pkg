(function (root) {
  ...
  (function (global) {
    ...
    var BUILDIN_MODULE = { require: globalRequire, exports: 1, module: 1 };
    ...
    function globalDefine(id, dependencies, factory) {
      ...
      if (id) {
        ...
        wait4PreDefine[0] = { deps: dependencies, factory: factory };
      }
    }
    globalDefine.amd = {};
    ...
    function modPrepare(id) {
      ...
      each(deps, function (depId, index) {
        ...
        if (absId && !BUILDIN_MODULE[absId]) {
          if (idInfo.res) {
            resInfo = { id: depId, mod: absId, res: idInfo.res };
    ...
    function loadResource(pluginAndResource, baseId) {
      ...
      var resource = { id: pluginAndResource, state: MODULE_ANALYZED };
    ...
    function normalize(id, baseId) {
      ...
      if (resourceId) {
        var mod =
          modIs(moduleId, MODULE_DEFINED) && actualGlobalRequire(moduleId);
        resourceId =
          mod && mod.normalize
    ...
    function kv2List(source, keyMatchable, allowAsterisk) {
      ...
      for (var key in source) {
        if (source.hasOwnProperty(key)) {
          var item = { k: key, v: source[key] };
          list.push(item);
          if (keyMatchable) {
            item.reg =
              key === "*" && allowAsterisk ? /^/ : createPrefixRegexp(key);
          }
        }
      }
      return list;

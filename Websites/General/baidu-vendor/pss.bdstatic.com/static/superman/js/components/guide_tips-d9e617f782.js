define("superman/components/guide_tips", ["require", "exports"], function (
  require,
  _exports
) {
  "use strict";
  ...
  function createdElement(top, left, option) {
    ...
    var newClass = option.id.trim() + "-" + option.type;
    $("#" + option.id).append(
      '\n        <div class="guide-info-new '
        .concat(newClass, '" style="left: ')
        .concat(left, "px;top: ")
        .concat(top, 'px;">\n            <span>')
        .concat(
          txt,
  ...
  function showGuideTips(option) {
    ...
    var top = 0;
    var left = 0;
    var txtGap = option.txt && option.txt.length - txtLength;
    var txtNumber = txtGap && txtGap > 0 ? txtGap : 0;
    switch (option.type) {
      case "bottom":
  ...
  function skinGuideInit(islogin) {
    var hideSkinGuide = "0";
    try {
      hideSkinGuide = window.localStorage.getItem("hide_guide_skin_new") || "0";
    } catch (e) {}
  ...
  function userCenterInit(islogin) {
    var hideGuide = "0";
    try {
      hideGuide = window.localStorage.getItem("hide_guide_usercenter") || "0";
    } catch (e) {}
    ...
    if (!hideGuide || hideGuide !== "1") {
      showGuideTips({
        ...
        red_dot: false,
        time_off: new Date("2020/02/24 20:25:00"),
      });
    }
    var closeParam = {
      id: elementId,

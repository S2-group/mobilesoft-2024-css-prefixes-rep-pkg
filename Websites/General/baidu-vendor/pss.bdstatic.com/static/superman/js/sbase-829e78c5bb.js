...
$.extend({
  browser: {
    ...
    firefox: /firefox\/(\d+\.\d+)/i.test(navigator.userAgent)
      ? +RegExp["$1"]
      : undefined,
    ie: /msie (\d+\.\d+)|Trident/i.test(navigator.userAgent)
    ...
    isSogouIE:
      ...
      navigator.userAgent.toLowerCase().indexOf("se") > 0 &&
      navigator.userAgent.toLowerCase().indexOf("360se") == -1,
    ...
    return function (style) {
      ...
      var getLengthPx = function (str) {
        ...
        for (var i = 0, len = str.length; i < len; i++) {
          ...
          lengthPx +=
            charCode > 255 ? cnLength : getCharLength.call(this, str.charAt(i));
        }
        return lengthPx;
      };
        ...
        for (var i = 0, len = str.length; i < len; i++) {
          var charCode = str.charCodeAt(i),
            charLen =
              charCode > 255
                ? cnLength
                : getCharLength.call(this, str.charAt(i));
      ...
      return { substringByPx: substringByPx, getLengthPx: getLengthPx };
    };
  })(),
});
$.extend({
  url: {
    ...
    escapeSSL: function (url) {
      ...
      var sslurl =
        list.hasOwnProperty(matches[2]) && list[matches[2]] + matches[3];
      if (!sslurl) {
        new Image().src =
    ...
    escapeSymbol: function (source) {
      return String(source).replace(
        ...
        function (all) {
          return (
            "%" +
            (256 + all.charCodeAt()).toString(16).substring(1).toUpperCase()
          );
        }
      );
    },
...
$.extend({
  ajaxget: function (url, callback, istab) {
    ...
    if (
      !$.url.getQueryValue(url, "asyn") ||
      $.url.getQueryValue(url, "asyn") != 1
    ) {
      addJson["asyn"] = 1;
    }
    ...
    if (
      !$.url.getQueryValue(url, "super_frm") &&
      $.url.getQueryValue(location.href, "super_frm")
    ) {
      addJson["super_frm"] = $.url.getQueryValue(location.href, "super_frm");
    }
    if (
      !$.url.getQueryValue(url, "from_login") &&
      $.url.getQueryValue(location.href, "from_login")
    ) {
      addJson["from_login"] = $.url.getQueryValue(location.href, "from_login");
    }
    if (
      !$.url.getQueryValue(url, "from_reg") &&
      $.url.getQueryValue(location.href, "from_reg")
    ) {
      addJson["from_reg"] = $.url.getQueryValue(location.href, "from_reg");
    }
        ...
        if (res) {
          ...
          try {
            ...
            new Image().src =
              ...
              encodeURIComponent(url) +
              "&ldq=" +
              encodeURIComponent(res.slice(0, 200));
            if (res.indexOf("script") >= 0) {
              $.ajax({
                type: "POST",
                url: "/home/page/data/pageserver",
                data: { errno: 7006, isData: encodeURIComponent(res) },
              });
            }
          }
          errno = data.errNo || data.errno;
          if (errno === undefined) {
            callback && callback({ errNo: -1e3 });
            window._xman_exception &&
              _xman_exception.api("get", url, "-1000", url.split("?")[1]);
          }
          if (errno != 0) {
            callback && callback({ errNo: errno });
            window._xman_exception &&
              _xman_exception.api("get", url, errno, url.split("?")[1]);
          }
          if (errno == "403") {
            new Image().src =
              s_domain.baseuri +
  ...
  ajaxpost: function (url, data, callback) {
    if (typeof data != "string") {
      ...
      if (
        ...
        data["super_frm"] = $.url.getQueryValue(location.href, "super_frm");
      }
      if (
        !$.url.getQueryValue(url, "from_login") &&
        $.url.getQueryValue(location.href, "from_login")
      ...
      success: function (res, textStatus) {
        if (res) {
          ...
          if (data.errNo === undefined) {
            window._xman_exception &&
              _xman_exception.api("post", url, "-1000", pdata);
          }
          if (data.errNo && data.errNo != 0) {
            window._xman_exception &&
              _xman_exception.api("post", url, data.errNo, pdata);
          }
          if (data.errNo == "7007") {
...
$.ajaxSetup({
  beforeSend: function (xhr) {
    var qid = s_session.qid || "";
    var url = this.url || "";
    var isTuOrVoice =
      url.indexOf("graph.baidu.com") > -1 || url.indexOf("vse.baidu.com") > -1;
...
$.extend({
  localstorage: (function () {
    var isDate = function (o) {
      return (
        {}.toString.call(o) === "[object Date]" &&
        o.toString() !== "Invalid Date" &&
        !isNaN(o)
      );
    };
    var _guid = "SUPER_LS_PLUGIN_1",
      _status = { SUCCESS: 0, FAILURE: 1, OVERFLOW: 2 };
    function _getKey(key) {
      return (
        "_super_" +
        key.replace(/[_\s]/g, function (matcher) {
    ...
    function _createUserData() {
      $(document.body).append(
        $.formatString('<div id="#{id}" style="display:none;"></div>', {
          id: _guid + "-storage",
        })
      );
      ...
      return {
          ...
          var status = _status.SUCCESS,
            ...
            time =
              options && options.expires
                ? options.expires
                : new Date().getTime() + 365 * 24 * 60 * 60 * 1e3;
          if (isDate(time)) {
            time = time.getTime();
          } else {
        ...
        get: function (key) {
          ...
          try {
            ...
            return { status: _status.FAILURE, value: null };
          }
    ...
    function _createCookie() {
      return {
        ...
        get: function (key) {
          ...
          return { status: _status[val ? "SUCCESS" : "FAILURE"], value: val };
        },
        del: function (key, callback) {
          ...
          callback &&
            callback.call(me, _status[val ? "SUCCESS" : "FAILURE"], val);
        },
      };
    }
    return {
...
(function (F, baseUrl) {
  ...
  Module.get = function (name, basePath) {
  ...
  Module.prototype = {
    load: function () {
      ...
      var modname =
        "/" + this.type + "/" + Module.moduleFileMap[svnMod][this.name];
      ...
      if (
        $.inArray(modname, F._firstScreenCSS) >= 0 ||
        (F._useConfig && $.inArray(modname, F._firstScreenJS) >= 0)
      ) {
        if (isCSS) {
          setTimeout(function () {
            _this._loaded = true;
  ...
  function assetOnload(src, type, callback, reload) {
    ...
    var timer = setTimeout(function () {
      ...
      !reload &&
        assetOnload(src + "?v=" + new Date().valueOf(), type, cb, true);
  ...
  F.inherit = function () {
    ...
    var oinitialize = subpro.initialize;
    var initializes = [];
    var index = 1;
    var superclass;
    var superpro;
    while ((superclass = args[index++])) {
      superpro = $.isFunction(superclass)
        ? superclass.prototype
        : $.isPlainObject(superclass)
        ? superclass
        : null;
      if (superpro) {
        $.isFunction(superpro.initialize) &&
    ...
    me.fire = function (eventName, args) {
      ...
      if (arr) {
        ...
        $.each(arr, function (idx, handler) {
          args._result = handler.call(
            me,
            $.extend({ _ctx: { src: me } }, args)
          );
        });
      }
    };
...
F.module("lib/mod_evt", function (require, exports, ctx) {
  ...
  var fire = function (modName, eventName, eventArgs) {
    var tmpArr = modName.split("/"),
      ...
      submod = tmpArr[1],
      key = mod + "-" + eventName,
      eventResult = { handlerCount: 0, returnValue: true };
    if (handlers[key]) {
      eventArgs = eventArgs || {};
      var hs = handlers[key];
  ...
  var addEvent = function (modName, eventName, handler) {
    var tmpArr = modName.split("/"),
      ...
      submod = tmpArr[1],
      key = mod + "-" + eventName;
    h = { hnd: handler, mod: mod, submod: submod };
    if (handlers[key]) {
      handlers[key].push(h);
    } else {
      handlers[key] = [h];
...
F.module("lib/log", function (require, exports, ctx) {
  ...
  var buildInKeys = {
    ...
    logPortrait = s_session.logPortrait,
    logSrcDomain = 0,
    ubslogactids = {};
  var getBuildInValues = function (mod, submod) {
    var values = {
      ...
      utype: bUtype,
      superver: bSuperVer,
      portrait: logPortrait,
      logPortrait: bPortrait,
      glogid: bGlogId,
      type: 2011,
      pid: 315,
  ...
  var imgLogSrc = function () {
    return $.url.escapeSSL(
      "http://dj" + (++logSrcDomain % 3) + ".baidu.com/v.gif?"
    );
  };
  ...
  var logEventListener = function (evt) {
    ...
    if (
      params.logactid === "0200100000" ||
      (params.logactid === "0200100001" && params.clickType === "wordTopic")
    ) {
      imgLog("//www.baidu.com/home/hit/v.gif?" + $.url.jsonToQuery(params));
    }
    ctx.use("lib/mod_evt", function (evt) {
      var evtResult = evt.fire("lib/log", "beforeSendLog", {
        modEvt: evt,
        logParams: params,
      });
  ...
  var imgLogPC = function () {
    ...
    var optLog = { logactid: "1234567890", showTab: "10000", opType: "showpv" };
  ...
  var imgLogTC = function () {
    ...
    img.src =
      ...
      "&ssid=" +
      window.s_session.portrait +
      "&from=0&pu=sz%401320_2001%2Cta%40iphone_1_9.1_3_601" +
      "&qid=" +
      window.s_session.logId +
      "pc" +
      "&ct=10&cst=1&clk_from=exception&clk_info=pclog";
    img = null;
  };
  ...
  if (window.webb2 && randTool(1)) {
    ...
    webb.send("pf_comm", { qid: bds.comm.qid }, {}, null, { group: "showpv" });
  }
...
(function (global) {
  ...
  Thunder.prototype = {
    ...
    bindLogHook: function (mainDom, domHook) {
      ...
      mainDom &&
        mainDom.on("click", domHook || "[" + self.domHook + "]", function (e) {
...
F.module("superman:common/user_attr", function (require, exports, ctx) {
  ...
  var setAttr = function (name, value) {
    var oldValue = getAttr(name);
    if (typeof oldValue != "undefined" && oldValue != value) {
      var bsToken = ($("#bsToken") && $("#bsToken").val()) || "";
      $.ajax({
        url: "/home/data/setups?bsToken=" + bsToken,
        type: "POST",
        data: JSON.stringify({ props: [name], values: [parseValue(value)] }),
      ...
      function callback(data) {
        if (data.errno == 0) {
          ...
          ctx.fire("setUserAttr", { attr: name, state: "setSucc" });
        } else {
          ctx.fire("setUserAttr", { attr: name, state: "setFail" });
        }
      }
    }
  };
  var setMultiAttr = function (names, values) {
    ...
    if (_nLeng > 0) {
      if (isFromSameInterface) {
        $.ajaxpost(
          s_domain.baseuri + URLS.SUBMIT_STATUS[PropOrTips.name],
          { prop: _names.join(","), value: _values.join(",") },
          function (data) {
            if (data.errNo == 0) {
              for (var j = 0; j < _nLeng; j++) {
                userProp[PropOrTips.name][_names[j]] =
                  _values[j] == "on" ? true : false;
                ctx.fire("setUserAttr", { attr: _names[j], state: "setSucc" });
              }
            } else {
              for (var k = 0; k < _nLeng; k++) {
                userProp[PropOrTips.name][_names[k]] = _values[k];
                ctx.fire("setUserAttr", { attr: _names[k], state: "setFail" });
              }
            }
          }
        );
...
F.module("superman:common/select", function (require, exports, ctx) {
  ...
  var Select = S.Class.create(function (cfg) {
    ...
    var setLayerPos = function () {
      ...
      me.layer.css({ left: pos.left + left + "px", top: pos.top + top + "px" });
    };
    ...
    var bindLayer = function () {
      ...
      me.layer.bind("mouseleave", function (e) {
        me.mouseoverOpt &&
          me.mouseoverOpt[0] &&
          me.mouseoverOpt.remove("current");
        e.preventDefault();
        e.stopPropagation();
      });
    ...
    me.getResult = function () {
      return { value: me.hidden.val(), words: me.inputMask.html() };
    };
    me.hide = function () {
      me.select.hide();
      hideLayer();
    ...
    (function () {
      var html = [
        ...
        '<div id="' +
          me.inputMaskId +
          '" class="s-select-input-mask s-opacity-blank1 ' +
          cfg.customClass +
          '"></div><input type="input" class="s-select-input ' +
          cfg.customClass +
          '" value="' +
          me.words +
          '" id="' +
          me.inputId +
          '">',
        '<a class="s-select-arrow s-opacity-border3-left ' +
          cfg.customClass +
          '" id="' +
          me.arrowId +
          '" href="#" onclick="return false;" hidefocus></a></span>',
      ...
      $("#head").append(
        '<div id="' +
          me.layerId +
          '" class="s-select-layer  s-isindex-wrap ' +
          cfg.customClass +
          '"></div>'
      );
    ...
    $("#bottom_layer .s-bottom-show").click(function () {
      if (s_session.isLogin) {
        window.location.href =
          location.protocol + "//" + location.host + "/home/xman/show/liteoff";
        return;
      }
